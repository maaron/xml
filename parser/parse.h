...
namespace parse
{
    ...
    namespace operators
    {
        ...
        optional<expr_t> operator! (const expr_t& expr)
        {
            return optional<expr_t>();
        }

        template <typename expr_t>
        repetition<expr_t, 1> operator+ (const expr_t& expr)
        {
            return repetition<expr_t, 1>();
        }

        template <typename parser_t, typename token_t>
        complement< parser_t, token_t > operator~ (const single<parser_t, token_t>& lhs)
        {
            return complement< parser_t, token_t >();
        }
